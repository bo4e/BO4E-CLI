...
```
src/
├── lib.rs           # `Options`, `RustCrateOptions`, `clear_dir_if_exists` — no top-level `generate` anymore
...
│   ├── render.rs    # Per-block renderers + `render_object` orchestration (with discriminator detection + Default impl)
...
└── templates/
    ...
    └── rust/
        └── plain/   # Struct, ModRs, RootModRs (consumed by both rust-plain and rust-crate)
```
