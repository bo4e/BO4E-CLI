...
fn classify_all(
    schemas: &bo4e_schemas::models::schema_meta::Schemas,
) -> Vec<(SchemaModule, String)> {
    schemas
        .schemas()
        .iter()
        .map(|s| {
...
impl Executable for Edit {
    fn run(&self) -> Result<(), String> {
        clear_dir_if_needed(&self.output_dir, !self.no_clear_output).map_err(|e| e.to_string())?;
        ...
        if !self.no_dirty_version {
            ...
            cprint_normal!(
                "Marked output version as locally edited: {}",
                schemas.version
            );
        }
            ...
            for schema_rc in schemas.iter() {
                    ...
                    && let Some(version_prop) = obj.object.properties.get_mut("_version")
                {
                    let base = match version_prop {
                        bo4e_schemas::models::json_schema::SchemaType::StringSchema(s) => {
                            &mut s.base
                        }
                        ...
                        _ => {
                            cprint_verbose!(
                                "_version field has unexpected schema type, skipping default assignment"
                            );
                            continue;
                        }
                    };
...
mod tests {
    ...
    fn test_edit_sets_default_version() {
        ...
        assert!(
            ...
            "output should contain a default value"
        );
        assert!(
            text.contains("v202401.1.0"),
            "default should be the schema version"
        );
    }
    ...
    fn test_edit_keeps_clean_version_when_disabled() {
        ...
        assert!(
            !written.contains(".d"),
            "expected no dirty suffix in {written}"
        );
    }
}
